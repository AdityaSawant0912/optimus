...
`@optimus/node` provides SSR + Node helpers: build an `EvaluationContext`
...
round-trip. `evaluate()` in `@optimus/core` is pure and deterministic
...
doing nothing until configured — the same stance `@optimus/core` takes on
